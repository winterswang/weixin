...
class otherCli extends BaseCli{
    ...
    function addWxButton(){
        $systemSetting = Config::getConfig('system');
        $data = $systemSetting['test']['menuButton'];
        //print_r(json_encode($data));
        $appid =  $systemSetting['test']['appid'];
        $secret=$systemSetting['test']['secret']; 
        $token = WxApiTools::model()->getAccessToken($appid,$secret);
        $res = WxApiTools::model()->addMenuButton($data,$token);            
    }

    function wxApiQRImage(){
        $systemSetting = Config::getConfig('system');
        $appid =  $systemSetting['test']['appid'];
        $secret=$systemSetting['test']['secret'];             
        $token = WxApiTools::model()->getAccessToken($appid,$secret);
        ...
        while($num <=1000){
            ...
            if($res = WxApiTools::model()->getQRImage($res,'qr'.$num)){
    ...
    function getTrainResult(){
        ...
        print_r(json_decode(WxApiTools::model()->get($url)));       
    } 
}